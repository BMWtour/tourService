        ...
        switch (provider) {
            ...
            case "google":
                ...
                if (user == null) {         // 내 DB에 없으면 가입을 시켜줌
                    ...
                    user = User.builder()
                            .userId(uid).userPw(hashedPwd).userNickname(uname)
                            .regDate(LocalDate.now()).provider(provider).userImgUri(profileUrl)
                            .build();
                    userService.registerUser(user);
                    log.info("구글 계정을 통해 회원가입이 되었습니다. " + user.getUserNickname());
                }
                break;
            ...
            case "naver":
                ...
                if (user == null) {         // 내 DB에 없으면 가입을 시켜줌
                    ...
                    user = User.builder()
                            ...
                            .build();
                    userService.registerUser(user);
                    log.info("네이버 계정을 통해 회원가입이 되었습니다. " + user.getUserNickname());
                }
                break;
            ...
            case "kakao":
                ...
                if (user == null) {         // 내 DB에 없으면 가입을 시켜줌
                    ...
                    user = User.builder()
                            ...
                            .build();
                    userService.registerUser(user);
                    log.info("카카오 계정을 통해 회원가입이 되었습니다. " + user.getUserNickname());
                }
                break;
